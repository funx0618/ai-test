...
1. **运行测试**：使用 `runTests` 工具执行新增/修改的测试方法
2. **如果通过**：向用户报告测试通过，结束
3. **如果失败**：
   a. 使用 `testFailure` 工具获取失败详情
   b. 分析失败原因（定位器错误、等待超时、断言失败等）
   c. 修复代码（优先修改 Page/Flow 层，必要时修改测试）
   d. 重新运行测试
   e. 重复 a-d 直到通过，最多重试 **3 次**
4. **3 次仍未通过**：向用户报告失败原因和已尝试的修复方案，等待用户指示

### 场景 2：用户手动运行测试失败后

当用户手动点击运行按钮或在终端执行 pytest 命令后，如果测试失败：

1. **获取失败信息**：使用 `testFailure` 工具获取失败详情
2. **分析失败原因**：定位器错误、等待超时、断言失败等
3. **修复代码**：优先修改 Page/Flow 层，必要时修改测试
4. **重新运行测试**：使用 `runTests` 工具验证修复
